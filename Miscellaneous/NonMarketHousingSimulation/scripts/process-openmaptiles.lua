...
function way_function()
	...
	if building ~= "" then
		-- Skip underground-only structures
		local osm_layer = tonumber(Find("layer")) or 0
		local location  = Find("location")
		if osm_layer >= 0 and location ~= "underground" then
			Layer("building", true)
			SetBuildingHeightAttributes()
			local bin = Find("nycdoitt:bin")
			if bin ~= "" then Attribute("bin", bin) end
			SetMinZoomByArea()
		end
	end
