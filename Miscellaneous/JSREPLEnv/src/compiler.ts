...
function tagChr(chr: string, expectedTypes: Array<typeof ExpectedType>) {
  ...
  if (isAlpha(chr)) {
    tag.tags.push(ExpectedType.Alpha);
  }
  if (isNumeric(chr)) {
    tag.tags.push(ExpectedType.Numeric);
  }
  if (isWhiteSpace(chr)) {
    tag.tags.push(ExpectedType.WhiteSpace);
  }
  if (isNewLine(chr)) {
    {
      tag.tags.push(ExpectedType.NewLine);
    }
  }
  return tag;
}
